...
The resulting `fchain` list suggests that "aircon" and "insulation" should be fused jointly in the initial block followed by the remaining fusion variables in a preferred order. You may get slightly different results, since `blockchain()` relies on random cross-validation. We can then pass the suggested blocking and chaining strategy to `train()`, whose `y` argument accepts a list for the purposes of specifying variable blocks.

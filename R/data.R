...
#' A variety of data types are included (numeric, factor, logical). There are no
#' missing values (`NA`). Variable names have been standardized from the original
#' EIA microdata format.
#'
#' @format A tibble (`tbl_df`) with 5,686 rows and 124 variables:
#' \describe{
...
#'   \item{cooling_share}{Share of total energy used for cooling (AC and fans).}
...
#'   \item{rep_1, rep_2, ..., rep_96}{Replicate weights (rep_1 through rep_96) for uncertainty estimation.}
#' }
#'
#' @docType data
#' @keywords datasets
#'
#' @references
#' U.S. Energy Information Administration (EIA). (2015). *Residential Energy Consumption Survey (RECS)*.
#' \url{https://www.eia.gov/consumption/residential/data/2015/}
#'
#' @examples
#' data(recs)
#' head(recs)

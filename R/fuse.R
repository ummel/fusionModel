...
#' @param k Integer. Number of nearest neighbors to identify among the donor observations. Not applicable for categorical variables fused individually.
...
#' train(data = recs, y = "electricity", x = predictor.vars, file = "test_model.fsn")
#'
...
#'
#' any(sim1 == recs$electricity)
#' any(sim2 == recs$electricity)
#'
#' @export
...
fuse <- function(data,
  ...
  # Unzip all fusion model files to temporary directory
  zip::unzip(zipfile = file, exdir = td)
  ...
  rsize <- objectSize(rmat) # NOTE: This would be more accurate if it accounted for factor/integer/logical y-variable classes in eventual output
  ...
  # rsize * M: the maximum size of the results data.table that will be created (dout)
  # dsize: initial size of the prediction data (including fusion variable placeholder columns) prior to expansion
  # Resulting 'nimp' gives the integer number of implicates to process in each chunk
  ...
  if (n > 1) dmat <- dmat[rep(seq.int(N0), nimp[1]), ]
  gc()
  ...
  for (mstep in 1:nstep) {
    ...
    for (i in 1:length(pfixes)) {
      ...
      # LightGBM predictor variables - NO LONGER NECESSARY
      #xv <- meta$lgbpred[[i]]
  ...
  # Check number of output rows is correct
  stopifnot(nrow(dout) == M * N0)

...
#' @return If \code{M > 1}, a data frame with number of rows equal to \code{M * nrow(data)}. Integer column "M" indicates implicate assignment of each observation. Note that the ordering of recipient observations is consistent within implicates, so do not change the row order if using with \code{analyze()}.
#'
#' @examples
...
fuse <- function(data,
  ...
  fsize <- as.numeric(object.size(fmat)) / 1048576

  # Allocate the prediction matrix
  dmat <- cbind(dmat, fmat)
  ...
  # TO DO: May want to save intermediate results to disk if there isn't enough memory; in order to increase implicate chunk size
  ...
  for (mstep in 1:nsteps) {
    ...
    for (i in 1:length(pfixes)) {
      ...
      for (m in mods) {
        ...
        # The row subset is slower, so only perform if necessary
        if (any(zind)) {
        ...
        p <- as.data.table(p)
        #pcols <- if (is.matrix(p)) 1:ncol(p) else 1
      ...
      if (block | meta$ytype[[v[1]]] == "continuous") {
        ...
        zip::unzip(zipfile = file, files = x, exdir = td)  # !!!Could move elsewhere!!!

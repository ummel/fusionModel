...
train <- function(data,
  ...
  rpart.args <- list(cp = 0, xval = 0, minbucket = 50)  # Default rpart() arguments
  rpart.default <- TRUE
  if (!missing(...)) {
    rpart.default <- FALSE
  ...
  # Set function to use for parallel processing, if mc = TRUE
  if (mc) {
    mcFun <- parallel::mclapply
  ...
  # Output list for rpart() models
  models <- vector(mode = "list", length = length(yord))
  names(models) <- yord

  # Assemble dummy matrix for 'xvars'
  ...
  for (i in 1:length(yord)) {
    ...
    if (cont) {
      ...
      Qn <- 500  # Should be function control argument
    ...
    if (y %in% names(ycor)) {
      ...
      p <- suppressWarnings(cor(ranks[ind, ], yrank[ind, ]))[, 1]  # Will silently return NA if no variance in a particular column
      p <- p[!is.na(p)]
      ycor[[y]] <- cleanNumeric(p, tol = 0.001)
    }

    # Update the 'ranks' matrix
    # This is memory-inefficient for large datasets; should pre-allocate 'ranks', if possible

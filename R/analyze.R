...
#' @param implicates Data frame containing implicates of synthetic (fused) variables. Typically generated by \link{fuse}. The implicates should be row-stacked and identified by integer column "M".
...
#' @param replicate_weights Optional data frame where each column is a vector of survey replicate weights. \code{nrow(replicate_weights)} must equal \code{length(sample_weights)}. If \code{NULL}, standard errors are calculated without using replicate weights.
...
#' @param var.scale Scalar. Factor by which to scale the standard variance in the presence of replicate weights. This is determined by the survey design. The default (\code{var.scale = 4}) is appropriate for both RECS and ACS.
...
analyze <- function(formula,
  ...
  imp.est <- parallel::mclapply(1:mcnt, FUN = function(m) {

...
#' @param sample_weights Vector of primary sampling weights.
...
#'
#' @details At a minimum, the user must supply synthetic implicates (typically generated by \link{fuseM()}) and primary sampling weights. Inputs are checked for consistent dimensions.
#' @details If \code{implicates} contains only a single version of the data (see Examples), calculations proceed but with a warning to let user know that there is no variance across implicates.
...
#' @details If replicate weights are NOT provided, the standard errors of each implicate are calculated using variance within the implicate. For means (and totals), the ratio variance approximation of Cochran (1977) is used, as this is known to be a good approximation of bootstrapped SE's for weighted means (Gatz and Smith 1995). For proportions, a generalization of the unweighted SE formula is used (\href{https://stats.stackexchange.com/questions/159204/how-to-calculate-the-standard-error-of-a-proportion-using-weighted-data}{see here}). For regression coefficients, the standard error is calculated by \code{\link[stats]{summary.glm()}}.
#'
...
#' @references Gatz, Donald F., and Luther Smith. (1995). The Standard Error of a Weighted Mean Concentration — I. Bootstrapping vs Other Methods. \emph{Atmospheric Environment}, vol. 29, no. 11, 1185–1193.
#' @references Reiter, J.P. (2003). Inference for Partially Synthetic, Public Use Microdata Sets. \emph{Survey Methodology}, vol. 29, 181-189.
#'
#' @examples
...
analyze <- function(formula,
  ...
  # Note that this prioritizes 'implicates' when a variable is requested that is ALSO in 'static'
  ...
  imp.est <- pbapply::pblapply(1:mcnt, FUN = function(m) {
    ...
    if (type == "mean") {
      ...
      calcMean <- function(x) {
        ...
        data.table(metric = c("mean", "total"),
                   est = c(pe.mu, pe.tot),
                   se = c(se.mu, se.tot),
                   nobs = nrow(x))

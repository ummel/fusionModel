...
#'
#' @details Argument \code{"static"} must include both sample and replicate weights. The total uncertainty of any given analysis results from the combination of uncertainty across replicate weights and uncertainty across synthetic implicates. That is, uncertainty concerning the household weights in the recipient survey plus uncertainty concerning the value each household takes with respect to a particular variable (i.e. model uncertainty).
...
analyze <- function(formula,
  ...
  stopifnot({
    ...
    is.list(synthetic) | (is.data.frame(synthetic) & "M" %in% names(synthetic))
  ...
  # Convert synthetic list to data frame with column 'M' indicating the implicate
  # If 's
  if (!is.data.frame(synthetic)) {
    names(synthetic) <- 1L:length(synthetic)
    synthetic <- rbindlist(synthetic, idcol = "M")
  }

  # Add 'M' columns to 'synthetic' if it does not exist; assume there is only 1 implicate in this case
  if (!"M" %in% names(synthetic)) synthetic$M <- 1L

  # Determine variables used within 'formula'; will throw error if all required variables cannot be located
  dtemp <- cbind(synthetic[1, ], static[1, ])
  fvars <- names(get_all_vars(formula, data = dtemp))

  # Retain only necessary columns in the synthetic data
  ...
  # Note that this prioritizes 'synthetic' when a variable is requested that is ALSO in 'static'
  static <- static %>%
    select(any_of(setdiff(c(fvars, by), names(synthetic[[1]]))), weight, starts_with("rep_")) %>%
    mutate_if(is.numeric, as.double) %>%
    as.data.table()
  stopifnot(all(c("weight", "rep_1", "rep_2") %in% names(static)))

  # Safety checks
  # Check for equal number of rows for each 'M' and they match row count in 'static'
  syn.cnt <- synthetic[, .N, by = M]
  stopifnot(all(syn.cnt$N == nrow(static)))
  ...
  imp.est <- pbapply::pblapply(syn.cnt$M, FUN = function(m) {

    # Create implicate-specific data.table
    # NOTE: This is probably not efficient to have the weight columns tagging along in memory?
    dt <- cbind(synthetic[M == m, ], static)
  ...
  result <- imp.est %>%
    ...
    summarize(
      ...
      ubar = mean(.data$se ^ 2) * syn.N / donor.N,  # Within-implicate variance of estimate; ADJUSTED for sample size
  ...
  if (type == "mean") {
    ...
    pos <- all(!any(synthetic[[rvar]] < 0) & !any(static[[rvar]] < 0))  # Is the response variable strictly positive?
  ...
  if (nrow(syn.cnt) == 1) {
    ...
    if (nrow(syn.cnt) < 5) warning("Only ", nrow(syn.cnt), " implicates provided. Reliability of results increases with the number of implicates.")
  }

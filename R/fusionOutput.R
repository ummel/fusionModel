...
#' @param acs_year Integer. Year of the ACS microdata to be used as recipient.
#' @param fusion_vars Character. Vector of variables in the donor microdata to be fused to the ACS microdata recipient.
...
#' @param input_version Character. Version (i.e. date stamp) of the desired fusion inputs. Defaults to the latest available version.
#' @param output_version Character. Version (i.e. date stamp) to assign to the outputs. Defaults to today's date.
#' @param fsn Character. Optional path to a .fsn object created by a previous call to \code{fusionOutput()}. If specified, code attempts to use \code{fsn} to bypass the training step.
#' @param rlocation Data frame. Optional data frame to replace the donor input data imputed respondent locations with actual/disclosed locations specified in \code{rlocation}.
...
fusionOutput <- function(donor,
  ...
  cat("The fusion OUTPUT directory is:\n", dir.out, "\n\n")
  ...
  # Report the modification date and MD5 checksum for the spatial predictors file
  cat("Using 'geo_predictors.fst' file last modified:", format(file.info(gfile)$mtime, usetz = TRUE), "\n")
  cat("The MD5 checksum is:", tools::md5sum(gfile)[[1]], "\n")

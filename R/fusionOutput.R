...
#' @param test_mode Logical. If \code{test_mode = TRUE} (default), the result files are always saved within "/fusion_", faster hyperparameters are used for \code{\link[fusionModel]{train}}, and the internal validation step is skipped.
#' @param upload Logical. Should resulting local /output data files be uploaded to analogous location in remote Google Drive storage? Default is \code{FALSE}.
...
#' 1. **Fuse onto training data for internal validation**. Fuses multiple implicates to original donor training data using \code{\link[fusionModel]{fuse}}. Results saved to disk. If \code{test_mode = TRUE}, this step is skipped.
#' 1. **Run fusionModel::validate()**. Passes previous step's results to \code{\link[fusionModel]{validate}}. Results saved to disk. If \code{test_mode = TRUE}, this step is skipped.
...
#' 1. **Upload /output files to Google Drive**. If \code{upload = TRUE} and other requirements are met, uploads output data files to the analogous location in the remote Google Drive storage.
#' 1. **fusionOutput() is finished!** Upon completion, a log file named \code{"outputlog.txt"} is written to \code{/output} for reference.
#'
#' @return Saves resulting /output data files to appropriate local directory and (optionally) to remote Google Drive storage. Also saves a .txt log file alongside data files that records console output from \code{fusionOutput}.
#'
#' @examples
...
fusionOutput <- function(input,
  ...
  if (test_mode) {
    cat("Running in 'test' mode, so internal validation skipped.\n")
  } else {

    # Fuse multiple implicates to training data for internal validation analysis
    valid <- fusionModel::fuse(data = train.data,
                               fsn = fsn.path,
                               M = M,
    ...
    validation <- fusionModel::validate(observed = train.data,
                                        implicates = fusionModel::read_fsd(valid),
  ...
  suppressWarnings(rm(train.data, prep, valid, validation))

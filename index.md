...
[`?recs`](https://ummel.github.io/fusionModel/reference/recs.md) for
details). For real-world use cases, the donor and recipient data are
typically independent and vary in sample size. For illustrative
...
We create a fusion model using the
[`train()`](https://ummel.github.io/fusionModel/reference/train.md)
function. The minimal usage is shown below. See
[`?train`](https://ummel.github.io/fusionModel/reference/train.md) for
additional function arguments and options. By default, this results in a
“.fsn” (fusion) object being saved to “fusion_model.fsn” in the current
working directory.
...
path of the .fsn model to the
[`fuse()`](https://ummel.github.io/fusionModel/reference/fuse.md)
function. Each variable specified in `fusion.vars` is fused in the order
provided. By default,
[`fuse()`](https://ummel.github.io/fusionModel/reference/fuse.md)
...
[`fuse()`](https://ummel.github.io/fusionModel/reference/fuse.md)
generates a unique, probabilistic set of outcomes.
...
For this call to
[`train()`](https://ummel.github.io/fusionModel/reference/train.md), we
specify a set of hyperparameters to search over when training each
LightGBM gradient boosting model (see
[`?train`](https://ummel.github.io/fusionModel/reference/train.md) for
details). The hyperparameters can be used to tune the underlying GBM
models for better cross-validated performance. We also set `nfolds = 10`
(default is 5) to indicate the number of cross-validation folds to use.
Since this requires additional computation, the `cores` argument is used
...
`M` argument within
[`fuse()`](https://ummel.github.io/fusionModel/reference/fuse.md). Here
we generate 10 implicates; i.e. 10 unique, probabilistic representations
of what the recipient records might look like with respect to the fusion
variables.
...
> \[!IMPORTANT\] The
> [`analyze()`](https://ummel.github.io/fusionModel/reference/analyze.md)
> function in the `fusionModel` package documents and implements the
> point estimate and variance estimation methodology used in [Ummel et
...
> Methods](https://ummel.github.io/fusionACS/)). While
> [`analyze()`](https://ummel.github.io/fusionModel/reference/analyze.md)
> is retained for legacy support and replication purposes, users are
...
both from a coding and mathematical standpoint – the
[`analyze()`](https://ummel.github.io/fusionModel/reference/analyze.md)
...
[`analyze()`](https://ummel.github.io/fusionModel/reference/analyze.md)
automatically returns the proportions associated with each factor level.
...
[`analyze()`](https://ummel.github.io/fusionModel/reference/analyze.md).
For example, the following call calculates the mean value of
“natural_gas” and “square_feet”, the median value of “square_feet”, and
the sum of “electricity” (i.e. total consumption) and “insulation”
...
function provides a convenient way to perform internal validation tests
on synthetic variables that have been fused back onto the original donor
data. This allows us to assess the quality of the underlying fusion
model; it is analogous to assessing model skill by comparing predictions
to the observed training data.

[`validate()`](https://ummel.github.io/fusionModel/reference/validate.md)
compares analytical results derived using the multiple-implicate fusion
output with those derived using the original donor microdata. By
performing analyses on population subsets of varying size,
[`validate()`](https://ummel.github.io/fusionModel/reference/validate.md)
estimates how the synthetic variables perform for analyses of varying
difficulty/complexity. It computes fusion variable means and proportions
for subsets of the full sample – separately for both the observed and
fused data – and then compares the results.
...
Next, we pass the `sim` results to
[`validate()`](https://ummel.github.io/fusionModel/reference/validate.md).
The argument `subset_vars` specifies that we want the validation
exercise to compare observed (donor) and simulated point estimates
across population subsets defined by “income”, “age”, “race”, and
“education”. See
[`?validate`](https://ummel.github.io/fusionModel/reference/validate.md)
for more details.
...
The
[`validate()`](https://ummel.github.io/fusionModel/reference/validate.md)
output includes ggplot2 graphics that helpfully summarize the validation
results. For example, the plot below shows how the observed and
simulated point estimates compare, using median absolute percent error
as the performance metric. We see that the synthetic data do a very good
job reproducing the point estimates for all fusion variables when the
population subset in question is reasonably large. For smaller subsets –
i.e. more difficult analyses due to small sample size – “square_feet”,
“natural_gas”, and “electricity” remain well modeled, but the error
increases more rapidly for “aircon” and “insulation”. This information
is useful for understanding what kind of reliability we can expect for
particular variables and types of analyses, given the underlying fusion
model and data.
